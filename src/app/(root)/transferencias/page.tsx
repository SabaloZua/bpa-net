...
export default function Page() {
  ...
  async function handleTranferencia(data: intraSchema) {
    try {
      setIsLoading(true);
      const dataset = {
        descricao: transferType === "intrabank" ? 'Transferencia intrabancaria' : "Transferencia interbancaria",
        idconta: Number(localStorage.getItem("idConta")),
        ...data
      }

      //console.log(dadoadd)
      const url = transferType === "intrabank" ? "/trasacao/transferenciaintra" : "/trasacao/transferenciainter"
      const response = await api.post(url, dataset);
      toast.success(response.data.message, {
        action: {
          label: 'Comprovativo',
          onClick: async () => window.open(`http://localhost:5000/pdf/comprovativo/${response.data.idtransacao}`, '_blank')
        }
      });
    } catch (erro) {
  ...
  return (
    <div>
       <Cabecalho Titulo='Transferências' subTitulo='pague onde quiser'/>

      <div className='bg-gray-100 rounded-xl p-4'>
        <div className="bg-white rounded-xl shadow-md overflow-hidden mb-6">

          <form onSubmit={handleSubmit(handleTranferencia)}>
            <div className="px-6 py-5 sm:p-6">
              ...
              <div className="mb-6">
                ...
                <RadioGroup value={transferType} onChange={(e) => setTransferType(e.target.value)} className="mt-2">
              ...
              <div className="space-y-5">
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      {...register("benefeciario")} required
                    />
                  </div>
                </div>
                ...
                {transferType === 'intrabank' ? (
                  <div>
                    <label htmlFor="destination-account" className="block text-sm font-medium text-gray-700 mb-2">
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="block w-full pl-10 pr-3 py-3 border border-gray-300 rounded-lg bg-white text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-400 ease-in-out"
                        {...register("contadestino")} required
                      />
                    </div>
                  </div>
                ) :
            ...
            <div className="px-6 py-4 bg-gray-50 border-t border-gray-200">
              <button disabled={isLoading} type='submit' className="w-full bg-blue-600 hover:bg-blue-700 text-white py-3 px-4 rounded-lg flex items-center justify-center font-medium transition-colors duration-200 shadow-md">
                {isLoading ? (
                  <TailSpin
                    height="25"
                    width="25"
